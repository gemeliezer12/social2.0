...
<body>
    ...
    <main class="sidebar-margin-left main-body" style="">
        ...
        if(!empty($implodeFollowing)){
            $query = $pdo->prepare(
            "SELECT 'repostPost' as type, p.userID, postID as articleID, p.content, r.dateCreated FROM posts p JOIN reposts r ON p.postID=r.repostedPost WHERE r.userID in ($implodeFollowing) AND p.userID!=?
            UNION 
            SELECT 'repostComment' as type, c.userID, commentID as articleID, c.content, r.dateCreated FROM comments c JOIN reposts r ON c.commentID=r.repostedComment WHERE r.userID in ($implodeFollowing) AND c.userID!=?
            UNION
