...
if(isset($_POST["submit"])){
    ...
    $query->execute();
    if($query->rowCount() > 0){
        $query = $pdo->prepare("UPDATE commentinteractions SET liked=? WHERE postedID=? AND commentID=?");
        $query->bindValue(1, true);
        $query->bindValue(2, $postedID);
        $query->bindValue(3, $commentID);
        $query->execute();
        $query = $pdo->prepare("UPDATE commentinteractions SET dateLiked=? WHERE postedID=? AND commentID=?");
